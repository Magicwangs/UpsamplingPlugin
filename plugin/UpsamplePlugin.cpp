...
UpsamplePlugin::UpsamplePlugin(const std::string name, int scale_factor, bool align_corners)
    : mLayerName(name), mAlignCorners(align_corners), mScaleFactor(scale_factor)
{
...
void UpsamplePlugin::configurePlugin(const Dims *inputs, int nbInputs, const Dims *outputs, int nbOutputs, const DataType *type, const DataType *type2, const bool *inputIsBroadcast, const bool *outputIsBroadcast, PluginFormat format, int maxBatchSize)
{
    //printf("-----UpsamplePlugin::configureWithFormat\n");
    // Validate input arguments
    assert(nbOutputs == 1);
    assert(type[0] == DataType::kFLOAT);
...
IPluginV2Ext *UpsamplePlugin::clone() const
{
...
IPluginV2Ext *UpsamplePluginCreator::createPlugin(const char *name, const PluginFieldCollection *fc)
{
...
IPluginV2Ext *UpsamplePluginCreator::deserializePlugin(const char *name, const void *serialData, size_t serialLength)
{
